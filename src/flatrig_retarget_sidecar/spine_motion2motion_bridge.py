...
from flatrig_retarget_sidecar.spine_import import SpineBoneRecord, SpinePackage, load_spine_package
...
def build_bvh_joint_layout(
    ...
    if len(root_like_bones) > 1:
        synthetic_matching_name = _sanitize_bvh_name("flatrig_root", used_names)
...
def _add_export_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("source", help="Spine JSON/SKEL/ZIP source path")
    ...
    parser.add_argument("--metadata", help="optional sidecar JSON path")
    parser.add_argument("--fps", type=float, default=30.0, help="sampling fps")
    parser.add_argument(
        "--positions",
        dest="positions_mode",
        choices=["all", "root"],
        default="all",
        help="BVH channel mode: keep positions on all joints or root only",
    )


def _add_mapping_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("source", help="source Spine JSON/SKEL/ZIP path")
    parser.add_argument("target", help="target Spine JSON/SKEL/ZIP path")
    ...
    parser.add_argument("--max-pairs", type=int, default=12, help="maximum sparse pairs")
    parser.add_argument(
        "--min-chain-score",
        type=float,
        default=0.38,
        help="minimum score for chain pairing",
    )


def _add_prepare_pair_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("source", help="source Spine JSON/SKEL/ZIP path")
    parser.add_argument("target", help="target Spine JSON/SKEL/ZIP path")
    parser.add_argument("--source-animation", required=True, help="source animation name")
    parser.add_argument("--target-animation", required=True, help="target animation name")
    parser.add_argument(
        "--output-dir", required=True, help="output directory for the prepared pair"
    )
    parser.add_argument("--fps", type=float, default=30.0, help="sampling fps")
    parser.add_argument(
        "--positions",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Bridge Spine clips into Motion2Motion-friendly assets."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    export_parser = subparsers.add_parser("export-bvh", help="export one Spine animation to BVH")
    _add_export_args(export_parser)

    mapping_parser = subparsers.add_parser(
        "export-mapping",
        help="export a sparse Motion2Motion mapping with BVH-safe joint names",
    )
    _add_mapping_args(mapping_parser)

    prepare_pair_parser = subparsers.add_parser(
        "prepare-pair",
        help="export source/target BVH plus sparse mapping for Motion2Motion",
    )
    _add_prepare_pair_args(prepare_pair_parser)

    args = parser.parse_args(argv)

    if args.command == "export-bvh":
            ...
            json.dumps(
                {
                    ...
                    "animation": metadata.animation_name,
                    "frame_count": metadata.frame_count,
                    "root": metadata.root_bvh_name,
                    "motion2motion_non_root_translate_bones": metadata.motion2motion_non_root_translate_bones,
                    "ignored_scale_bones": metadata.ignored_scale_bones,
                },
                indent=2,
            )
        )
    ...
    if args.command == "export-mapping":
        ...
        payload = build_exported_motion2motion_mapping(
            source,
            target,
        ...
        if args.diagnostics:
            ...
            diagnostics_path.write_text(
                json.dumps(payload.get("metadata") or {}, indent=2) + "\n", encoding="utf-8"
            )
        print(
            json.dumps(
                {
                    "output": str(output_path.resolve()),
                    "pair_count": len(payload["mapping"]),
                    "root_joint": payload["root_joint"],
                },
                indent=2,
            )
        )
    ...
    if args.command == "prepare-pair":
        ...
        source_bvh_path = output_dir / "source.bvh"
        source_meta_path = output_dir / "source.meta.json"
        target_bvh_path = output_dir / "target.bvh"
        target_meta_path = output_dir / "target.meta.json"
        ...
        manifest = {
            "source": {
                "spine": source.source_label,
                "animation": args.source_animation,
                "bvh": str(source_bvh_path.resolve()),
                "metadata": str(source_meta_path.resolve()),
                "frame_count": source_metadata.frame_count,
                "motion2motion_non_root_translate_bones": source_metadata.motion2motion_non_root_translate_bones,
                "ignored_scale_bones": source_metadata.ignored_scale_bones,
            },
            "target": {
                "spine": target.source_label,
                "animation": args.target_animation,
                "bvh": str(target_bvh_path.resolve()),
                "metadata": str(target_meta_path.resolve()),
                "frame_count": target_metadata.frame_count,
                "motion2motion_non_root_translate_bones": target_metadata.motion2motion_non_root_translate_bones,
                "ignored_scale_bones": target_metadata.ignored_scale_bones,
            },
            ...
            json.dumps(
                {
                    ...
                    "source_bvh": str(source_bvh_path.resolve()),
                    "target_bvh": str(target_bvh_path.resolve()),
                    "mapping": str(mapping_path.resolve()),
