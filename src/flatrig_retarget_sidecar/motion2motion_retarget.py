...
def retarget_bvh_pair_auto(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="m2m_auto_", dir=str(ROOT_DIR / "workflow")
    ) as temp_dir_value:
        temp_dir = Path(temp_dir_value)
...
def _resolve_target_package_with_exemplar(
    ...
    *,
    ...
    synthetic_name = "__flatrig_rest__"
...
def main() -> None:
    parser = argparse.ArgumentParser(description=f"Probe and run {M2M_LABEL}.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    probe_parser = subparsers.add_parser("probe", help="probe the backend")
    del probe_parser

    inspect_bvh_parser = subparsers.add_parser("inspect-bvh", help="inspect one BVH source")
    inspect_bvh_parser.add_argument("source_bvh")

    bvh_parser = subparsers.add_parser("retarget-bvh", help="retarget one BVH pair")
    bvh_parser.add_argument("source_bvh")
    bvh_parser.add_argument("target_bvh")
    bvh_parser.add_argument("mapping_file")
    bvh_parser.add_argument("--output", required=True)
    bvh_parser.add_argument("--device", default=None)
    bvh_parser.add_argument("--matching-alpha", type=float, default=None)

    auto_bvh_parser = subparsers.add_parser(
        "retarget-bvh-auto",
        help="retarget one BVH pair with automatic sparse mapping generation",
    )
    auto_bvh_parser.add_argument("source_bvh")
    auto_bvh_parser.add_argument("target_bvh")
    auto_bvh_parser.add_argument("--output", required=True)
    auto_bvh_parser.add_argument("--device", default=None)
    auto_bvh_parser.add_argument("--matching-alpha", type=float, default=None)
    ...
    if args.command == "probe":
        print(json.dumps(asdict(probe_motion2motion_backend()), indent=2))
        return

    if args.command == "inspect-bvh":
        print(json.dumps(inspect_bvh_source(args.source_bvh), indent=2))
        return
    ...
    if args.command == "retarget-bvh-auto":
        result = retarget_bvh_pair_auto(
            args.source_bvh,
            args.target_bvh,
            output_bvh=args.output,
            device=args.device,
