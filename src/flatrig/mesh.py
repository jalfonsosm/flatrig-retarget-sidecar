...
def transfer_3d_weights_to_2d(eval_obj, eval_mesh, bone_name_to_index):
    """Transfer original 3D vertex-group weights directly into 2D bone indices.
    
    Extracts from eval_mesh to ensure weights map correctly after topology-changing
    modifiers like Decimate.
    """
    group_names = {group.index: group.name for group in eval_obj.vertex_groups}
    all_weights = []
