...
def test_probe_command_can_skip_scene_backend(monkeypatch, capsys) -> None:
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        sys, "argv", ["flatrig-retarget-sidecar", "probe", "--no-include-scene-backend"]
    )
    ...
    assert "scene_backend" not in payload
