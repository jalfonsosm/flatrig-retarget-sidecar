...
Public Python sidecar shared by FlatRig applications.
...
- A stable CLI contract that multiple FlatRig clients can call.
...
`tools/install_torch_runtime.py` pins FlatRig to a single Torch version (`2.9.1`) while selecting the right wheel flavor for the platform:

- Apple Silicon: default wheels with `MPS`
- Windows/Linux: CUDA wheels by default
- CPU-only override: `FLATRIG_TORCH_FLAVOR=cpu`

`tools/install_motion2motion_backend.py` reuses the root `.venv` by default so FlatRig clients and Motion2Motion resolve the same Torch runtime. Pass `--dedicated-venv` only if you explicitly want a separate backend environment.

## CLI

```bash
python cli/flatrig_retarget.py probe
python cli/flatrig_retarget.py probe --no-include-scene-backend
